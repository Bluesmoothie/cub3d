...
/*   Updated: 2025/04/29 15:31:46 by sithomas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (i < ctx->map.pixelmap)
	{
		...
		while (j < ctx->map.pixelmap)
		{
			if (x == (int)ctx->player.posx && y == (int)ctx->player.posy)
				render_map_pixel(&ctx->map, 0x00FFFFFF, ctx->map.pixelmap * x
					+ i, ctx->map.pixelmap * y + j);
			else if (ctx->map.map[x][y] == -2 || ctx->map.map[x][y] == 6)
				render_map_pixel(&ctx->map, 0x00F1C40F, ctx->map.pixelmap * x
					+ i, ctx->map.pixelmap * y + j);
			else if (ctx->map.map[x][y] == -1 || ctx->map.map[x][y] == 1)
				render_map_pixel(&ctx->map, 0x00566573, ctx->map.pixelmap * x
					+ i, ctx->map.pixelmap * y + j);
			else if (ctx->map.map[x][y] == 0 || ctx->map.map[x][y] > 1)
				render_map_pixel(&ctx->map, 0x009c640c, ctx->map.pixelmap * x
					+ i, ctx->map.pixelmap * y + j);
			j++;
		}
		i++;
